...
int StackFuncCtor(StackFunc_t* stk, int capasity, int line, const char* StackFuncname, const char* funcname) {
    ...
    #ifdef CANARY
        stk->data = (StackFuncElement_t* ) calloc((size_t) capasity + 2, sizeof(StackFuncElement_t));
    #else
        stk->data = (StackFuncElement_t* ) calloc((size_t) capasity,     sizeof(StackFuncElement_t));
    #endif
...
int StackFuncRealloc(StackFunc_t* stk, int line) {
    ...
    #ifdef CANARY
    StackFuncElement_t* stkreal = (StackFuncElement_t* ) realloc(stk->data, (2 * (size_t) stk->capasity + 2) * sizeof(stk->data[0]));
    #else
    StackFuncElement_t* stkreal = (StackFuncElement_t* ) realloc(stk->data, (2 * (size_t) stk->capasity) * sizeof(stk->data[0]));
    #endif
